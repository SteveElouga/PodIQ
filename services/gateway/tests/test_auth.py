...
def make_info(auth_header: str | None = None) -> SimpleNamespace:
    ...
    return SimpleNamespace(context=SimpleNamespace(request=SimpleNamespace(headers=headers)))


def make_jwt_response(valid: bool, user_id: str = "", error: str = "") -> SimpleNamespace:
...
class TestRequireAuth:
    ...
    def test_validate_jwt_called_with_extracted_token(self):
        ...
        with patch("app.auth.auth_client.validate_jwt", return_value=response) as mock_fn:
            from app.auth import require_auth
