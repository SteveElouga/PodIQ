...
def make_parsed_manifest(
    name="my-app", kind="Deployment", raw_config='{"image":"nginx:latest"}'
):
...
class TestScanManifestMutation:
    def _run(
        ...
        with (
            ...
            patch(
                "app.graphql.mutations.scan_manifest.ai_client.scan_manifest",
                return_value=scan_result,
            ),
        ):

            from app.graphql.mutations.scan_manifest import (
                _scan_manifest as scan_manifest,
            )

            return scan_manifest(
                info=None, yaml_content=yaml_content, manifest_type=manifest_type
            )
    ...
    def test_risks_are_mapped_correctly(self):
        risks = [
            make_risk(
                ...
                description="No DATABASE_URL",
                fix="Add it",
            ),
            make_risk(
                severity="medium",
                category="probe",
                description="No readiness probe",
                fix="Add probe",
            ),
        ]
        result = self._run(
            scan_result=make_scan_result(risk_level="block", risks=risks)
        )
    ...
    def test_safe_result_has_no_risks(self):
        result = self._run(
            scan_result=make_scan_result(risk_level="safe", summary="OK", risks=[])
        )
    ...
    def test_ai_scan_receives_raw_config(self):
        ...
        with (
            ...
            scan_manifest(
                info=None, yaml_content=SAMPLE_YAML, manifest_type="Deployment"
            )
...
class TestScanManifestAuth:
    ...
    def test_block_risk_level_propagated(self):
        result = TestScanManifestMutation()._run(
            scan_result=make_scan_result(risk_level="block", summary="Dangerous config")
        )
    ...
    def test_risk_fix_is_propagated(self):
        ...
        result = TestScanManifestMutation()._run(
            scan_result=make_scan_result(risks=risks)
        )
